...
%
%
% Syntax: DHC_E(dataPath, outPath, outName)
%
% Inputs:
%        dataPath:
...
%   Hyperparameter-free and Explainable Whole Graph Embedding. arXiv
%   preprint arXiv:2108.02113
%   [#] co-first author
%
% Hao Wang, UESTC, Chengdu, March 3, 2021, h.wang.psyc(at)gmail.com
%--------------------------------------------------------------------------
if nargin < 3
    error('Check your input parameters, at least 3 parameters are needed')
end
...
for ig = 1:numGraphs
    ...
    fid = fopen(allGraph(ig).name, 'r');
    Dir_data = textscan(fid, '', 'Delimiter', ',', 'CollectOutput', true);
    fclose(fid);
    g = Dir_data{1};
...
% fileID = fopen([outName '.csv'], 'w');
% fmt = ['%s, ', [repmat('%f, ', 1, NumNumericColumns-1), '%f\n']];
% for ig = 1:numGraphs
%     fprintf(fileID, fmt, allGraph(ig).name, Embeddings(ig,:));
% end
% fclose(fileID);
